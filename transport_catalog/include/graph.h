...
namespace Graph {
  ...
  struct Edge {
    Edge() = default;
    Edge(VertexId f, VertexId t, Weight w) : from(f), to(t), weight(w) {}
    VertexId from;
    VertexId to;
    Weight weight;
  };
  ...
  class DirectedWeightedGraph {
   ...
   public:
    ...
    DirectedWeightedGraph(std::vector<Edge<Weight>> e,
                          std::vector<IncidenceList> il)
        : edges_(std::move(e)), incidence_lists_(std::move(il)) {}
